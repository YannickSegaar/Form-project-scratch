...
<script lang="ts">
	...
	$: selectedDakType = $formData.dakType
		? { label: $formData.dakType, value: $formData.dakType }
		: undefined;
	$: selectedStroomAansluiting = $formData.stroomAansluiting
		? { label: $formData.stroomAansluiting, value: $formData.stroomAansluiting }
		: undefined;
...
<form method="POST" class="mx-auto flex max-w-md flex-col" use:enhance use:focusTrap={isFocused}>
	<AppBar class="appbar-custom rounded-lg p-3">
		...
		<svelte:fragment slot="lead">
			...
			<img src="/Protium Quickscan Logo.png" alt="Company Logo" class="logo-in-appbar" />
		</svelte:fragment>
	</AppBar>
	<Stepper
		stepTerm="Stap"
		...
		buttonCompleteLabel="Verzenden"
	>
		<!-- Step 1: Postcode, Huisnummer -->
		<Step regionHeader="region-header-custom">
			<svelte:fragment slot="header"
				>Voor welke locatie wilt u de Quickscan uitvoeren?
			</svelte:fragment>
			<svelte:fragment slot="navigation"><div style="display: none;"></div></svelte:fragment>
			<!-- YRS: Deze navigation slot leeg laten zodat "Terug" button voor eerste stap niet getoond wordt -->
			<div class="first-step-fields-container">
				<!-- This is the new container div for the form fields -->
				<!-- Postcode -->
				<Form.Field {form} name="postcode" class="form-field">
					<Form.Control let:attrs>
						<div class="flex flex-col">
							...
							<Input
								{...attrs}
								class="placeholder-custom select-input-field"
								type="postcode"
					...
					<Form.Control let:attrs>
						<div class="flex flex-col">
							...
							<Input
								...
								class="placeholder-custom select-input-field"
								type="huisnummer"
								bind:value={$formData.huisnummer}
								placeholder="1 "
							/>
						</div>
					</Form.Control>
		...
		<Step regionHeader="region-header-custom">
			...
			<div class="step-2-container">
				...
				<div class="parent-flex-container">
					<Form.Field {form} name="dakoppervlak_toggle" class="child-flex-container">
						<Form.Control let:attrs>
							...
							<div class="space-y-2">
								<!-- Adjust 'space-y-2' to manage the space between elements -->
								<Form.Label>Dakoppervlak</Form.Label>
								<Form.Description>
									<p style="font-size: 12px;">
										Volgens de teken tool is de dakgrootte <strong>{roofsizeDrawing} m²</strong>
									</p>
									<p style="font-size: 12px;">Klopt dit niet?</p>
									<p style="font-size: 12px;">
										Gebruik dan de schakelaar om dit handmatig in te vullen.
									</p>
								</Form.Description>
							</div>
						...
						<Form.Control let:attrs>
							<div class="flex flex-col">
								...
								<Input
									...
									type="number"
									placeholder="... m²"
									bind:value={$formData.dakoppervlakManual}
									on:input={handleInput}
									disabled={!$formData.dakoppervlak_toggle}
								/>
							</div>
						</Form.Control>
						<Form.Description>Vul hier zelf uw dakoppervlak in</Form.Description>
						<Form.FieldErrors class="error-message" />
					</Form.Field>
				</div>
				...
				<div class="horizontal-flex">
					<!-- Flex container for horizontal fields -->
					<!-- DAKTYPE -->
						...
						<Form.Control let:attrs>
							<div class="flex flex-col">
								...
								<div class="select-background">
									<!-- YRS: Class zodat select dropdown menu niet meer doorzichtig is -->
									<Select.Root
										selected={selectedDakType}
										onSelectedChange={(v) => {
											v && ($formData.dakType = v.value);
										}}
										required
									>
										<Select.Trigger {...attrs} class="flex items-center">
											<!-- Use flex and items-center to align icon with text inside the select -->
											<!-- Icon inside the select trigger -->
											<!-- <span class="material-symbols-outlined icon">roofing</span> -->
											<Select.Value placeholder="Kies uw daktype" class="select-placeholder" />
										</Select.Trigger>
										<Select.Content class="select-background">
											<!-- YRS: Class zodat select dropdown menu niet meer doorzichtig is -->
						...
						<Form.Control let:attrs>
							<div class="flex flex-col">
								...
								<div class="select-background">
									<!-- YRS: Class zodat select dropdown menu niet meer doorzichtig is -->
									<Select.Root
										selected={selectedStroomAansluiting}
										onSelectedChange={(v) => {
											v && ($formData.stroomAansluiting = v.value);
										}}
										required
									>
										<Select.Trigger {...attrs} class="flex items-center">
											...
											<!-- <span class="material-symbols-outlined icon">electrical_services</span> -->
											<Select.Value
												placeholder="Kies uw stroomaansluiting"
		...
		<Step regionHeader="region-header-custom">
			...
			<div class="step-3-container">
				...
				<Form.Field {form} name="naam" class="form-field full-width">
					<!-- YRS: Apply full-width class voor step 3-->
					<Form.Control let:attrs>
						<div class="flex flex-col">
							<!-- Use flex-col for vertical stacking -->
							<div class="mb-2 flex items-center">
								...
								<span
									...
									style="margin-bottom: 0.1rem; font-size: 1.55rem;">person</span
								>
								<!-- Inline style for vertical adjustment -->
							...
							<Input
								...
								class="placeholder-custom select-input-field"
								type="naam"
								bind:value={$formData.naam}
								placeholder="Protium NL"
							/>
						</div>
					</Form.Control>
				...
				<!-- TELEFOONNUMMER -->
				<div class="flex-row">
					<!-- YRS: Use the new class for side-by-side layout voor stap 3 -->
					<Form.Field {form} name="telefoonNummer" class="flex-field">
						<!-- YRS: new class "flex-field" voor stap 3 -->
						<Form.Control let:attrs>
							<div class="flex flex-col">
								<!-- Use flex-col for vertical stacking -->
								<div class="mb-2 flex items-center">
								...
								<Input
									...
									class="placeholder-custom select-input-field"
									type="tel"
									bind:value={$formData.telefoonNummer}
									placeholder=" "
								/>
							</div>
						</Form.Control>
					...
					<Form.Field {form} name="email" class="flex-field">
						<!-- YRS: new class "flex-field" voor stap 3 -->
						<Form.Control let:attrs>
							<div class="flex flex-col">
								<!-- Use flex-col for vertical stacking -->
				...
				<Form.Field {form} name="klantOpmerkingen" class="form-field full-width">
					<!-- YRS: nieuwe class voor stap 3 -->
					<Form.Control let:attrs>
						<div class="flex flex-col">
							<!-- Use flex-col for vertical stacking -->
							<div class="mb-2 flex items-center">
							...
							<Textarea
								...
								class="placeholder-custom select-input-field h-32 resize-none"
				...
				<Form.Field {form} name="privacyAkkoord" class="form-field full-width">
					<Form.Control let:attrs>
						<div style="display: flex; align-items: center;">
							<Checkbox
								{...attrs}
								bind:checked={$formData.privacyAkkoord}
								class="select-input-field"
							/>
							<Form.Label style="font-size: 12px;"
								>&nbsp;&nbsp;Ik ga akkoord met de verwerking van de bovenstaande gegevens</Form.Label
							>
					...
					<div class="full-width" style="font-size: 12px;">
						...
						<a
							href="https://protium.nl/privacy-policy"
							target="_blank"
							rel="noopener noreferrer"
							style="color: blue; text-decoration: underline;">privacyverklaring</a
						>.
					</div>
				</Form.Field>
				...
				<Form.Button class="special-button">
					Verzenden
					<img
						src="/Protium Favicon Yellow.png"
						alt="Protium Logo"
						class="logo-inside-special-button-right"
					/>
					<img
						src="/Protium Favicon Yellow.png"
						alt="Protium Logo"
						class="logo-inside-special-button-left"
					/>
				</Form.Button>
  ...
  <img src="/Protium Favicon Yellow.png" alt="Protium Logo" class="stepper-complete-logo right" />
				...
				{#if $message}
					<div class:success-banner={$page.status == 200}>{$message}</div>
				{/if}
			</div>
		</Step>
	</Stepper>
