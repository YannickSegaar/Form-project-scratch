...
$: selectedDakType = $form.dakType ? { label: $form.dakType, value: $form.dakType } : undefined;
$: selectedStroomAansluiting = $form.stroomAansluiting ? { label: $form.stroomAansluiting, value: $form.stroomAansluiting } : undefined;
      
  // Reactive assignment based on toggle and manual input
      $: if ($form.dakoppervlak_toggle && $form.dakoppervlakManual !== undefined) {
      // If toggle is enabled and manual input is provided, use it
      $form.dakOppervlak = $form.dakoppervlakManual;
  } else {
      // Otherwise, fallback to the roofsizeDrawing value
      $form.dakOppervlak = roofsizeDrawing;
  }
  ...
  $: {
      ...
      $form.dakoppervlakManual = Number(dakOppervlakInput);
  }
...
<Form.Field {form} name="postcode" class="form-field">
  <Form.Control let:attrs>
    <div class="flex flex-col"> 
      ...
      <Input {...attrs} class="placeholder-custom select-input-field" type="postcode" bind:value={$form.postcode} placeholder="1234 AA" />
    </div>
  </Form.Control>
...
<Form.Field {form} name="huisnummer" class="form-field">
  <Form.Control let:attrs>
    <div class="flex flex-col"> 
      ...
      <Input {...attrs} class="placeholder-custom select-input-field" type="huisnummer" bind:value={$form.huisnummer} placeholder="1 " />
    </div>
  </Form.Control>
  <Form.FieldErrors class="error-message" />
</Form.Field>
...
<div class="parent-flex-container">
  <Form.Field {form} name="dakoppervlak_toggle" class="child-flex-container">
    <Form.Control let:attrs>
      ...
      <Switch includeInput {...attrs} bind:checked={$form.dakoppervlak_toggle} />
    </Form.Control>
  </Form.Field>
        ...
        <div class="flex flex-col">
          <div class="flex items-center mb-2">
            <span class="material-symbols-outlined icon">
              {$form.dakoppervlak_toggle ? 'lock_open' : 'lock'}
            </span>
            <Form.Label>Dakoppervlak in m²</Form.Label>
          </div>
          <Input {...attrs} class="placeholder-custom select-input-field" type="number" placeholder="... m²" bind:value={$form.dakoppervlakManual} on:input={handleInput} disabled={!$form.dakoppervlak_toggle} />
        </div>
      </Form.Control>
      <Form.Description>
        ...
        <Form.Field {form} name="dakType" class="form-field">
            <Form.Control let:attrs>
                <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                    ...
                    <Select.Root selected={selectedDakType} onSelectedChange={(v) => { v && ($form.dakType = v.value); }} required>
                    ...
                    <input hidden bind:value={$form.dakType} name={attrs.name} />
                </div>
            </Form.Control>
    ...
    <Form.Field {form} name="stroomAansluiting" class="form-field">
        <Form.Control let:attrs>
            <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                ...
                <div class="select-background"> <!-- YRS: Class zodat select dropdown menu niet meer doorzichtig is -->
                <Select.Root selected={selectedStroomAansluiting} onSelectedChange={(v) => { v && ($form.stroomAansluiting = v.value); }} required>
                  <Select.Trigger {...attrs} class="flex items-center"> <!-- Use flex and items-center to align icon with text inside the select -->
                        <!-- Icon inside the select trigger -->
                        <!-- <span class="material-symbols-outlined icon">electrical_services</span> -->
                ...
                <input hidden bind:value={$form.stroomAansluiting} name={attrs.name} />
            </div>
        </Form.Control>
    ...
    <Form.Field {form} name="naam" class="form-field full-width"> <!-- YRS: Apply full-width class voor step 3-->
        <Form.Control let:attrs>
            <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                ...
                <Input {...attrs} class="placeholder-custom select-input-field" type="naam" bind:value={$form.naam} placeholder="Protium NL" />
            </div>
        </Form.Control>
        ...
        <Form.Field {form} name="telefoonNummer" class="flex-field"> <!-- YRS: new class "flex-field" voor stap 3 -->
            <Form.Control let:attrs>
                <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                    ...
                    <Input {...attrs} class="placeholder-custom select-input-field" type="tel" bind:value={$form.telefoonNummer} placeholder=" " />
                </div>
            </Form.Control>
    ...
    <Form.Field {form} name="email" class="flex-field"> <!-- YRS: new class "flex-field" voor stap 3 -->
        <Form.Control let:attrs>
            <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                ...
                <Input {...attrs} class="placeholder-custom select-input-field" type="email" bind:value={$form.email} placeholder="hallo@protium.nl" />
            </div>
        </Form.Control>
        ...
        <Form.Control let:attrs>
            <div class="flex flex-col"> <!-- Use flex-col for vertical stacking -->
                ...
                <Textarea
                    ...
                    bind:value={$form.klantOpmerkingen}
                />
            </div>
            <!-- <Form.Description>
...
<Form.Field {form} name="privacyAkkoord" class="form-field full-width">
  <Form.Control let:attrs>
      <div style="display: flex; align-items: center;">
          <Checkbox {...attrs} bind:checked={$form.privacyAkkoord} class="select-input-field" />
          <Form.Label style="font-size: 12px;">&nbsp;&nbsp;Ik ga akkoord met de verwerking van de bovenstaande gegevens</Form.Label>
          <input name={attrs.name} value={$form.privacyAkkoord} hidden />
      </div>
  </Form.Control>
  <Form.FieldErrors class="error-message" />
